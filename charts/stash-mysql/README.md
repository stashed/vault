...
$ helm repo update
$ helm install stash-mysql-v2021.01.21 appscode/stash-mysql -n kube-system --version=v2021.01.21
```
...
To install the chart with the release name `stash-mysql-v2021.01.21`:
...
To uninstall/delete the `stash-mysql-v2021.01.21`:

```console
$ helm delete stash-mysql-v2021.01.21 -n kube-system
```
...
| image.tag        | Tag of the image that is used to backup/restore MySQL database. This is usually same as the database version it can backup. | `v2021.01.21`       |
...
```console
$ helm install stash-mysql-v2021.01.21 appscode/stash-mysql -n kube-system --version=v2021.01.21 --set image.registry=stashed
```
...
```console
$ helm install stash-mysql-v2021.01.21 appscode/stash-mysql -n kube-system --version=v2021.01.21 --values values.yaml
```
